package com.example.administrator.mazegame.Model;
import java.util.ArrayList;
import java.util.List;

public class MazeGame {
	private static final int MAZE_SIZE_WIDTH  = 15;
	private static final int MAZE_SIZE_HEIGHT = 20;
	private static int CAT_PRODUCE_DURATION;
	private static int NUM_CHEESE_TO_COLLECT;
	private int NUM_CAT;
	...
	public MazeGame(int NUM_CHEESE_TO_COLLECT) {
		this.NUM_CHEESE_TO_COLLECT = NUM_CHEESE_TO_COLLECT;
		this.CAT_PRODUCE_DURATION = 30;
	...
	public void addCat(){
		List<CellLocation> cells = new ArrayList<>();
		cells.add(LOCATION_TOP_RIGHT);
		cells.add(LOCATION_BOTTOM_LEFT);
		cells.add(LOCATION_BOTTOM_RIGHT);
		NUM_CAT++;
		cats.add(new Cat(this, cells.get(NUM_CAT%3)));
	}
	...
	public void recordPlayerMove(MoveDirection move) {
		...
		movementCount++;
		if(movementCount%CAT_PRODUCE_DURATION==0){
			addCat();
			movementCount=0;
		}
		...
		if (isCheeseAtLocation(playerLocation)) {
			numCheeseCollected++;
			if(numCheeseCollected<NUM_CHEESE_TO_COLLECT){
				placeNewCheeseOnBoard();
				if(NUM_CAT*30-movementCount<0){
					score+= NUM_CAT*100;
				}else{
					score+= (NUM_CAT*100+(NUM_CAT*30-movementCount));
				}
			}
		}
	...
	public int getCountUntilNextCat(){
		return CAT_PRODUCE_DURATION-movementCount;
	}

	public boolean isRevealed(){
		return maze.isAllRevealed();
	}
	public void incrementCatProduceDuration(){
		this.CAT_PRODUCE_DURATION = 1000000000;
	}
}
